...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        feedVC.tabBarItem.image = UIImage(systemName: "archivebox.circle.fill")
        feedVC.title = "所有反馈"
        let nav1 = UINavigationController(rootViewController: feedVC)
        
        let personalPage = PersonalPageViewController()
        personalPage.title = "个人中心"
        personalPage.tabBarItem.image = UIImage(systemName: "person.fill")
        let nav2 = UINavigationController(rootViewController: personalPage)
